...
上面的flag主要是用来标志是否是作为“根节点”（因为路径不要求root-to-leaf，因此所有的节点都有可能成为“根节点”或者“叶节点”）以及是否作为子节点（不包括孙贝节点）。因为我们希望如果是子节点的话，我们把它作为可能的路径开头，但是不能是孙节点也这样处理，因为会重复。
...
那么当到达节点3更节点2的时候，就会有上面的来各种情况在做几乎相同的计算，唯一不同的地方就是presum可能不一样。
